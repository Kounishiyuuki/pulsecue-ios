...
3. **プロバイダの API キーを iOS アプリに埋め込まない**：プロバイダの長期 API キーは公開アプリのバイナリに含めない。`xcconfig`（git 管理外）はキーを git から外すだけで、ビルド時にはバイナリへ焼き込まれ抽出可能なので、それ自体は保護にならない。本番のプロバイダ秘密はサーバ側にのみ置き、iOS が扱うのは短期トークンまでとする（`Docs/credential-strategy.md` / `Docs/photo-ai-provider-strategy.md` 参照）。キーは本リポジトリにもコミットしない。
...
- [ ] プロバイダ認証はサーバ側 proxy / 短期トークン方式で扱う（クライアントに長期キーを埋め込まない。`Docs/credential-strategy.md` 参照）
